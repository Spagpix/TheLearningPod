...
#define WEBGL_OR_LEGACY
    	...
    	#define _WorldSpaceCameraPos _wsEyePos[unity_StereoEyeIndex]
		#endif
#endif
    ...
    float4 _wsEyePos[2];
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			#if LIQUID_VOLUME_DEPTH_AWARE_PASS
			float frontDist = tex2D(_VLFrontBufferTexture, uv).r;
				t1 = min(t1, frontDist);
			#endif
			
			#if LIQUID_VOLUME_IRREGULAR
			float backDist = tex2D(_VLBackBufferTexture, uv).r;
				t1 = min(t1, backDist);
