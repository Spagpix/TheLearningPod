...
public static bool FORCE_GLES_COMPATIBILITY = false;
		...
		public struct LiquidLayer
		{
			...
			public void SetDefaults ()
			{
				amount = 0.1f;
				density = 1f;
				color = new Color (UnityEngine.Random.value, Random.value, Random.value, 0.5f);
		...
		[SerializeField]
		bool _vrUseCenterCameraPos;

		public bool vrUseCenterCameraPos {
			get { return _vrUseCenterCameraPos; }
			set {
				if (_vrUseCenterCameraPos != value) {
					_vrUseCenterCameraPos = value;
					UpdateMaterialProperties ();
				}
			}
		}
		...
		float _pointLightInsideAtten = 0f;
		...
		public void OnWillRenderObject ()
		{
			...
			// Pass eye world positions
			if (wsEyePos == null) {
				wsEyePos = new Vector4[2];
			}

			if (_vrUseCenterCameraPos || !UnityEngine.XR.XRSettings.enabled) {
				wsEyePos [0] = wsEyePos [1] = Camera.current.transform.position;
			} else {
				...
				wsEyePos [0] = m.MultiplyPoint (-offset);
				wsEyePos [1] = m.MultiplyPoint (offset);
			}
			liqMat.SetVectorArray ("_wsEyePos", wsEyePos);
		...
		void FixedUpdate ()
		{
			...
			liqMat.SetFloat ("_MurkinessSpeed", murkinessSpeed);
		...
		void UpdateBubblesProperties ()
		{
			...
			liqMat.SetVector ("_BubblesData", new Vector3 (_bubblesScale, bubblesVerticalSpeedAcum, _bubblesBrightness));
		}
		...
		void UpdateLightEffects ()
		{
			...
			if (liqMat != null) {
				liqMat.SetFloat ("_PointLightInsideAtten", _pointLightInsideAtten);
				liqMat.SetColorArray ("_PointLightColor", pointLightColorBuffer);
				liqMat.SetVectorArray ("_PointLightPosition", pointLightPositionBuffer);
			}
		...
		void RefreshMaterialProperties ()
		{
			...
			liqMat.SetFloat ("_DitherStrength", _ditherStrength);
			...
			if (_noiseVariation != currentNoiseVariation || requireBubblesUpdate) {
				...
				if (noise3DTex [currentNoiseVariation] == null) {
					noise3DTex [currentNoiseVariation] = Instantiate<Texture3D> (Resources.Load<Texture3D> ("Textures/Noise3D" + currentNoiseVariation.ToString ()));
				}
				...
				if (tex3d != null) {
					...
					if (_bubblesAmount > 0 && _detail.isMultiple ()) {
						...
						liqMat.SetTexture ("_NoiseTexUnwrapped", null);
					}
		...
		void UpdateLevels (bool updateShaderKeywords = true)
		{
			...
			if (!_ignoreGravity && _rotationLevelBias > 0 && thisLevel > 0) {
				...
				float maxLevel = Mathf.Clamp01 (levelMultipled + 0.5f);
				float minLevel = Mathf.Clamp01 (levelMultipled - 0.5f);
				float resArea = 0;
				for (int i = 0; i < 12; i++) {
				...
				thisLevel = Mathf.Lerp (levelMultipled, thisLevel, _rotationLevelBias);
			} else {
				if (levelMultipled <= 0)
			...
			if (_detail.isMultiple ()) {
				liqMat.SetFloat ("_SizeWorld", size.y);
			}
...
//			buf.ClearRenderTarget (true, true, new Color (0.9882353f, 0.4470558f, 0.75f, 0f), 1f);
		...
		public bool GetSpillPoint (out Vector3 spillPosition, out float spillAmount, float apertureStart = 1f)
		{
			...
			spillAmount = 0;
			if (mesh == null || vertices == null || levelMultipled <= 0)
				return false;
		...
		public void CenterPivot (Vector3 offset, bool closeMesh)
		{
			...
			if (closeMesh) {
				List<Vertex> hullVertices = new List<Vertex> (mesh.vertices.Length);
				for (int k = 0; k < mesh.vertices.Length; k++) {
					hullVertices.Add (new Vertex (mesh.vertices [k]));
				}
		...
		void UpdateLayersNow ()
		{
			...
			for (int k = 0; k < _liquidLayers.Length; k++) {
				_liquidLayers [k].amount = Mathf.Clamp01 (_liquidLayers [k].amount);
				_liquidLayers [k].layerName = "Layer " + k;
			...
			for (int k = 0; k < _liquidLayers.Length; k++) {
				...
				for (int j = 0; j < _liquidLayers.Length; j++) {
					if (_liquidLayers [j].density == maxDensity) {
						...
						baseLevel += _liquidLayers [j].amount;
					}
				}
			...
			for (int k = 0; k < sortedLayersCount; k++) {
				...
				if (_liquidLayers [i].miscible) {
					...
					while (_liquidLayers [i].miscible && _liquidLayers[i].density == groupDensity) {
						float amount = _liquidLayers [i].amount + 0.0001f;
						...
						temp.density += _liquidLayers [i].density * amount;
						temp.amount += amount;
					...
					// Combine
					float mixedDensity = temp.density / temp.amount;
					float mixedAmount = Mathf.Clamp01 (temp.amount / mixedDensity);
					for (int l = k; l < j; l++) {
						i = sortedLayers [l];
						...
						_liquidLayers [i].mixedAmount = mixedAmount; // _liquidLayers[i].amount * _liquidLayers[i].density / mixedDensity;
		...
		{
			liqMat.SetColorArray ("_LayersColors", layersTextureColors);
			liqMat.SetColorArray ("_LayersColors2", layersTextureColors2);
			liqMat.SetVectorArray ("_LayersProperties", layersProperties);
		}
		...
		void SetLayersTextures ()
		{
			...
			liqMat.SetTexture ("_LayersPropertiesTex", layersPropertiesTex);
			layersColorsTex.SetPixels (layersColorsTexContents);
			layersColorsTex.Apply ();
			liqMat.SetTexture ("_LayersColorsTex", layersColorsTex);
			layersColors2Tex.SetPixels (layersColors2TexContents);
			layersColors2Tex.Apply ();
			liqMat.SetTexture ("_LayersColors2Tex", layersColors2Tex);
		}
		...
		void AddBubbles (Texture3D tex)
		{
			...
			if (useUnwrappedTexture) {
				...
				liqMat.SetTexture ("_NoiseTexUnwrapped", noise3DUnwrapped);
			}
