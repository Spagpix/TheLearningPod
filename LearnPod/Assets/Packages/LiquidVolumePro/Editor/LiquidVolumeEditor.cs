...
namespace LiquidVolumeFX
{
	...
	public class LiquidVolumeEditor : Editor
	{
		...
		SerializedProperty topology, detail, subMeshIndex, depthAware, irregularDepthDebug, depthAwareOffset, depthAwareCustomPass, depthAwareCustomPassDebug, ignoreGravity, reactToForces, doubleSidedBias, rotationLevelBias;
		SerializedProperty level, levelMultipler, liquidColor1, liquidColor2, liquidScale1, liquidScale2, alpha, emissionColor, emissionBrightness;
		...
		SerializedProperty vrUseCenterCameraPos;
		SerializedProperty pointLightParams, pointLightsEnabled, pointLightsScatteringAmount, pointLightsIntensity;
		...
		bool shaderFeatureScattering, shaderFeatureSmoke, shaderFeatureBubbles;
		...
		void OnEnable ()
		{
			...
			vrUseCenterCameraPos = serializedObject.FindProperty ("_vrUseCenterCameraPos");
		...
		public override void OnInspectorGUI ()
		{
			...
			if (detailed != (int)DETAIL.DefaultNoFlask && detailed != (int)DETAIL.Smoke && detailed != (int)DETAIL.MultipleNoFlask) {
				...
				if (expandSection [FLASK_SETTINGS]) {
					...
					EditorGUILayout.PropertyField (flaskThickness, new GUIContent ("Thickness", "Flask thickness."));
			...
			if (expandSection [ADVANCED_SETTINGS]) {
				...
				EditorGUILayout.PropertyField (vrUseCenterCameraPos, new GUIContent ("VR Use Center Cam", "Enable this option to use same camera position in VR Multi-Pass whe computing ray direction. This option reduces unfocused bubbles and other details in VR."));
				...
				if (shaders != null && shaders.Length > 0) {
					...
					if (expandSection [SHADER_VARIANTS]) {
						...
						EditorGUILayout.EndHorizontal ();
						EditorGUILayout.Separator ();
						EditorGUILayout.HelpBox ("You can disable these options to make shaders a bit faster on slow devices. The shader code will be modified when you click 'Update Shaders'.", MessageType.Info);
		...
		{
			if (lv.transform.localRotation == lv.transform.rotation) {
